...
StyleGuide.prototype.add = function(opts, fn){
  ...
  if (!opts.type && !opts.default) {
    throw error('must have a type or default', opts.filename, opts.id, opts.code);
  }

  if (opts.default) {
    var def = opts.type = opts.default;
    if (this.defaults[def]) {
      throw error('must have only 1 default for type ' + def, opts.filename, opts.id, opts.code);
    }
    this.defaults[def] = opts;
  }

  opts.type = opts.type || opts.default;
  opts.fn = fn;
